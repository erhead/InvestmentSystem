...
; (function ($, window, document, undefined) {
    ...
    $.ics = {
        ...
        dialog: {
             ...
             * возможности браузера с урезанной функциональностью.
             *
             * @param options Параметры окна: текст сообщение, заголовок окна, функция обратного вызова.
             */
            ...
            modal: function (options) {
                ...
                var newTbRemove = window.tb_remove = function () {
                    // Вызов пользовательского обработчика закрытия окна c
                    // возможностью отмены.
                    var callbackResult = { cancel: false, deferreds: [] };
                    if ($.isFunction(options.callback)) {
                ...
                if (options.href) {
                    ...
                    var $div = $('<div style="display: none;">').append($('<div>').html(options.content || '')).attr('id', uniqueId).appendTo('body');
        ...
        */
        triggerEvent: function (target, eventName, data, callback) {
            ...
            $.when.apply(null, data.deferreds).then(function () {
                if (!event.isDefaultPrevented() && !data.stop) {
                    callback();
                }
            });
        },
